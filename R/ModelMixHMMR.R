...
ModelMixHMMR <- setRefClass(
  ...
  methods = list(
    ...
    summary = function() {
      ...
      cat("\n")
      cat("\n")
      cat(paste0("MixHMMR model with K = ", paramMixHMMR$K, ifelse(paramMixHMMR$K > 1, " clusters", " cluster"),
                 " and R = ", paramMixHMMR$R, ifelse(paramMixHMMR$R > 1, " regimes", " regime"), ":"))
      cat("\n")
      cat("\n")

      tab <- data.frame("log-likelihood" = statMixHMMR$loglik, "nu" = paramMixHMMR$nu, "AIC" = statMixHMMR$AIC,
                        "BIC" = statMixHMMR$BIC, "ICL" = statMixHMMR$ICL1, row.names = "", check.names = FALSE)
